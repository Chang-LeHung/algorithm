...
从上面堆的结构容易知道，堆是一种二叉树结构，如果当前堆中有元素$N$个，则向堆中插入一个元素的时间复杂度为$O(log(N))$，它在下沉的时候，数据交换的次数不会大于$log(N)$。如果一个数组（堆）中有$N$个元素，那么它需要进行$N$次，根元素和堆的最后一个元素进行交换，然后数据进行下沉，每一次下沉的数据交换次数不会$log(N)$，而且越往后交换的次数距离$log(N)$越大，即交换的次数越来越少，因此堆排序的最大时间复杂度为$O(log(N))$。
